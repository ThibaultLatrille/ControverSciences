module MajHelper
  ...
  def maj_v_9
    ActiveRecord::Base.transaction do
      User.find_each do |user|
        notifs = NotificationSelection.where(user_id: user.id).count + user.notifications_typo
        user.update_columns(nb_notifs: notifs)
      end
      Credit.find_each do |credit|
        credit.update_frame
      end
      FrameCredit.find_each do |frame_credit|
