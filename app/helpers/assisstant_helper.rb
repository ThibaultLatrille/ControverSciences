module AssisstantHelper
  ...
  def update_all_profils
    ...
    UserDetail.all.each do |user_detail|
      ...
      count = Timeline.where(user_id: user_id).count
      user_detail.profil[1] = count > 0 ? Timeline.where(user_id: user_id ).sum(:score) / Math.sqrt(count) : 0.0
      count = Frame.where(user_id: user_id ).count
      user_detail.profil[2] = count > 0 ? Frame.where( user_id: user_id ).sum(:score) /  Math.sqrt(count) : 0.0
      count = Summary.where(user_id: user_id ).count
      user_detail.profil[3] =  count > 0 ? Summary.where(user_id: user_id ).sum(:score) /  Math.sqrt(count) : 0.0
      count = Reference.where(user_id: user_id ).count
      user_detail.profil[4] = count > 0 ? Reference.where(user_id: user_id ).sum(:star_most) /  Math.sqrt(count) : 0.0
      ...
      if count > 0
        ...
        user_detail.profil[5] =  score / Math.sqrt(count)
      else
        user_detail.profil[5] = 0.0
      end
      user_detail.profil[6] = Float(Like.where(user_id: user_id ).count)
      query = Comment.where(user_id: user_id )
      ...
      user_detail.profil[7] = Float(Summary.where(user_id: user_id ).sum(:balance) +
          Frame.where(user_id: user_id ).sum(:balance) + balance)
      user_detail.profil[8] = Float(Credit.where(user_id: user_id ).count +
          FrameCredit.where(user_id: user_id ).count +
          Vote.where(user_id: user_id ).count)
      user_detail.profil[9] = User.find( user_id).my_patches
    ...
    UserDetail.all.each do |user_detail|
      max.keys.each do |key|
        if max[key] > 0
          user_detail.profil[key.to_s] = Math.log(10 + Float(user_detail.profil[key.to_s])) / Math.log(10 + max[key])
        else
