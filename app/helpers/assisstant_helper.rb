module AssisstantHelper
  ...
  def update_all_profils
    ...
    UserDetail.find_each do |user_detail|
      ...
      user_detail.profil[8] = Float(Credit.where(user_id: user_detail.user_id ).sum(:value) +
          FrameCredit.where(user_id: user_detail.user_id ).sum(:value) +
          Vote.where(user_id: user_detail.user_id ).sum(:value))
  ...
  def selection_events
    Reference.select(:id, :slug).find_each do |reference|
      for field in 0..7 do
        update_best_comment(reference.id, field)
      end
    end
    Timeline.select(:id, :slug).find_each do |timeline|
