class PrivateTimelinesController < ApplicationController
  ...
  def create
    ...
    if count == 0
      ...
      flash.now[:success] = t('controllers.private_invit', count: count)
    end
  ...
  def set_index
    ...
    @user_names = User.order(:name).where(activated: true).where.not( id: @private_timelines.map{ |u| u.user_id } ).pluck(:name, :id)
  end
